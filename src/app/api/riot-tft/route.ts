...
import { RiotTFTQueueData } from "@/models/riot";
...
export async function GET() {
  ...
  try {
    const res = await fetch(`https://na1.api.riotgames.com/tft/league/v1/by-puuid/${RIOT_PUUID}`, {
      method: "GET",
      headers: {
    ...
    console.error(err);
    return NextResponse.json({ error: "Failed to fetch Riot API TFT data" }, { status: 502 });
  }
}